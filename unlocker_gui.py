...
class UnlockerApp(ctk.CTk):
    ...
    def run_process(self):
        try:
            # 1. Asegurar que el directorio de configs de anadius exista
            os.makedirs(APPDATA_DIR, exist_ok=True)

            # 2. Copiar config.ini del unlocker al AppData (NECESARIO antes de instalar el DLL)
            self.log_status("Copiando configuración del Unlocker...")
            self.install_local_file(CONFIG_INI, APPDATA_DIR)

            # 3. Instalar el DLL del Unlocker en EA App/Origin vía setup.bat
            self.log_status("Instalando el Unlocker en EA App/Origin...")
            self.run_setup_bat_install()

            # 4. Configurar The Sims 4 si está marcado
            ...
            # 5. Configurar The Sims 3 si está marcado
    ...
    def run_setup_bat_install(self):
        """Ejecuta setup.bat con el argumento 'install' para copiar el DLL al directorio de EA App/Origin."""
        ...
        try:
            # Usamos 'install' para que setup.bat copie el version.dll directo sin mostrar menú.
            # Nota: setup.bat pide privilegios de administrador internamente si los necesita.
            process = subprocess.Popen(
                ["cmd", "/c", bat_path, "install"],
            ...
            # Código de retorno negativo indica error real; 0 = éxito
            if process.returncode < 0:
                raise Exception(f"setup.bat terminó con código de error: {process.returncode}\nVer log en: {log_path}")
    ...
    def check_and_update_ini(self, filename, url):
        """Descarga el .ini desde GitHub y lo instala. Si no hay internet, usa el local de respaldo."""
        ...
        try:
            ...
            # Sin internet: usa el archivo empaquetado como respaldo
    ...
    def install_local_file(self, filename, target_dir):
        """Copia un archivo empaquetado en el .exe al directorio destino indicado."""
        base_dir = get_base_dir()
        source_path = os.path.join(base_dir, filename)
        target_path = os.path.join(target_dir, filename)
        
        if not os.path.exists(source_path):
            raise FileNotFoundError(f"No se encontró el archivo '{filename}' integrado en el .exe")
